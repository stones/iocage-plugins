...
#iocage exec "$JAIL_NAME" git clone --depth 1 https://github.com/the-paperless-project/paperless.git /usr/local/www/paperless
...
cp ./paperless.rc  "/mnt/iocage/jails/$JAIL_NAME/root/usr/local/etc/rc.d/paperless"
# Copy site configuration
cp ./site.nginx  "$JAIL_FOLDER/$JAIL_NAME/root/usr/local/etc/nginx/nginx.conf"


#ADD NGINX FIX FOR STATIC FILES
